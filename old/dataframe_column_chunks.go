...
func (df *DataFrame) InsertRow(id int, row interface{}) {
	...
	if df.storageOnDisk {
		...
		// Remove the chunk from memory to keep the memory footprint small
		for colName := range df.Columns {
			df.Columns[colName] = df.Columns[colName][:0]
		}
	} else {
...
func (df *DataFrame) ReadRow(id int) (map[string]interface{}, error) {
	...
	// Clear the loaded chunk from memory to keep the memory footprint small
	if df.storageOnDisk {
		for colName := range df.Columns {
			df.Columns[colName] = df.Columns[colName][:0]
		}
...
func (df *DataFrame) SaveRowChunkToDisk(chunkIndex int) error {
	...
	// Ensure the end index does not exceed the length of the columns
	for _, colData := range df.Columns {
		if end > len(colData) {
			end = len(colData)
		}
	}

	// Save the chunk to disk using the correct method name
	err := df.SaveColumnChunkToDisk(chunkIndex)
	if err != nil {
		return err
	}
