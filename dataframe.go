...
func ReadFromCSVString(csvData string, v interface{}) (*DataFrame, error) {
	...
	for _, record := range data {
		...
		for i, header := range headers {
			field := elem.FieldByNameFunc(func(name string) bool {
				...
				return ok && strings.EqualFold(field.Tag.Get("json"), header)
			})
	...
	fmt.Println("vSlice", vSlice)

	//df, err := NewDataFrame(v)
	//if err != nil {
	//	return nil, err
	//}
	//
	...
	//	values := make([]interface{}, vSlice.Index(i).NumField())
	//	for j := 0; j < vSlice.Index(i).NumField(); j++ {
	//		values[j] = vSlice.Index(i).Field(j).Interface()
	//	}
	//	if err := df.Insert(elem, values); err != nil {
	//		return nil, err
	//	}
	//}
	fmt.Println("vSlice", vSlice)
	df := &DataFrame{StructType: reflect.TypeOf(v)}
